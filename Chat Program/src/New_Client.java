...
//로그인 Tab과 서버 연결을 하기위한 클라이언트 프로그램의 중추적인 클래스
 ...
 New_Client() //기본적으로 메인 로그인 GUI를 생성
 {
     ...
     pw.println("52273#!login"); //로그인했을 경우
      runClient(); //로그인이 허가가 나온 후 작동하는 함수
      pw.println("52273#!logout"); //로그아웃 구현
       socket.close();
     pw.println("Ting_Talk"); //로그인 후 ID를 입력해야됨
    ...
    public void run() {
     ...
     while(true) //무한루프로 외부에서 연결하는 소켓을 서버와 연결
     {
      ...
      else if(temp[0].charAt(0)=='!')//x번방의 대화목록 리스트를 서버에서 받아옴 !방번호#id1#id2#id3
      {
       ...
       System.arraycopy(users, 1, copyUsers, 0, users.length-1); //복사할 배열,복사 시작 인덱스, 대상바열, 복사할 갯수
 ...
 new New_Client();
  ...
  /*
  pw.println("52273#!logout"); //완전히 프로그램을 껏을경우
  pw.println("방번호#보낼메시지"); //일반적으로 문자를 해당 방번호에게 보내고 싶을경우
  pw.println("해당방번호#!exit"); //해당 방을 나갈려고 할 경우
