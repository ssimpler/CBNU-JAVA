...
public class FindPanel extends JFrame{
	...
	static JPanel getFindFriend(){
		...
		//JLabel logo = new JLabel(new ImageIcon(getClass().getResource("1.jpg"))); //팅톡 아이콘생성
...
/////////////////////////////////////////////////////////////////////////////////////
	    ActionListener actionListener = new ActionListener(){
			...
			public void actionPerformed(ActionEvent actionEvent){
			   ...
			   if(actionEvent.getSource()==txt1){ FriendInfo=actionEvent.getActionCommand(); } //텍스트필드에서 엔터를 누를시...이벤트
			   if(actionEvent.getSource()==FButton){ FriendInfo=txt1.getText(); } // 버튼클릭시 이벤트...
	            ...
	            try{
	               ...
	               if(findindex==0){  sql="select * from client_list where client_id like ?"; }//물음표에는 동적으로 변화하는 값을 넣기 위함
	               else if(findindex==1){  sql="select * from client_list where client_name like ?"; }//물음표에는 동적으로 변화하는 값을 넣기 위함
	               else if(findindex==2){  sql="select * from client_list where client_phone like ?"; }//물음표에는 동적으로 변화하는 값을 넣기 위함
	               else if(findindex==3){  sql="select * from client_list where client_email like ?"; }//물음표에는 동적으로 변화하는 값을 넣기 위함
		    ...
		    //상세정보 액션 이벤트
				        	...
				        	if(!InfoCut[0].equals("")){
				    		 ...
				    		 try {	        		  
				    		     ...
				    		     String sql1 = null;
				                    ...
				                    if(findindex==0){  sql1="select * from client_list where client_id like ?"; }//물음표에는 동적으로 변화하는 값을 넣기 위함
				                   else if(findindex==1){  sql1="select * from client_list where client_name like ?"; }//물음표에는 동적으로 변화하는 값을 넣기 위함
				                   else if(findindex==2){  sql1="select * from client_list where client_phone like ?"; }//물음표에는 동적으로 변화하는 값을 넣기 위함
				                   else if(findindex==3){  sql1="select * from client_list where client_email like ?"; }//물음표에는 동적으로 변화하는 값을 넣기 위함
					...
					if(actionEvent.getActionCommand() == "친구추가") {
			        	...
			        	if((!addCut[0].equals("")) && !addCut[0].equals(New_Client.getClientName())){ //빈칸을 클릭하거나 자신의 ID를 추가 할 경우 막음
			        		//즉 내가 클릭한 항목이 빈칸이 아니거나 내 ID와 같지 않다면...
			        	Connection con = null;
			        	
			        	try{
			        		 con =DriverManager.getConnection("jdbc:mysql://"+New_Client.DBIP+":3306/TingTalk","root","dnfxmfk8");			
